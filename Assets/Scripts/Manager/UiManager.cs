...
//                     arrowLeft.DOFade(1, 0);
    ...
    public void MadnessRedStart ( )
    {
        ...
        StartCoroutine (waitMadnessRed ( ));
    }

    IEnumerator waitMadnessRed ( )
    {
        PostProcessMadness.GetComponent<PostProcessVolume> ( ).enabled = true;
        GlobalManager.GameCont.postMadnessProfile.enabled = true;
        var volume = PostProcessManager.instance.QuickVolume (PostProcessMadness.layer, 100f, GlobalManager.GameCont.postMadnessProfile.profile.settings.ToArray ( ));
        volume.weight = 0f;

        Tween saveT = DOTween.To (( ) => volume.weight, x => volume.weight = x, 0.6f, .6f).SetLoops (-1, LoopType.Restart);
        ...
        saveT.Kill ( );
        DOTween.To (( ) => volume.weight, x => volume.weight = x, 0, .6f).OnComplete (( ) =>
        {
            PostProcessMadness.GetComponent<PostProcessVolume> ( ).enabled = false;
            GlobalManager.GameCont.postMadnessProfile.enabled = false;
        });
    }
