...
Shader "custom/character_toonshading" {
    Properties {
        ...
        _Outline_basevalue ("Outline_basevalue", Range(0, 0.005)) = 0.001723851
        _Outline_ratiodistancescale ("Outline_ratiodistancescale", Range(0, 0.005)) = 0.001
...
////// Specular:
                ...
                float3 node_5958 = lerp(node_9770,(node_9770*_gloss_color),(max(0,dot(lightDirection,i.normalDir))*pow(max(0,dot(i.normalDir,halfDirection)),exp2(lerp(1,11,_gloss)))));
                float3 diffuseColor = node_5958; // Need this for specular when using metallic
...
////// Emissive:
                float node_1010 = 0.0;
                float3 emissive = lerp(node_5958,float3(node_1010,node_1010,node_1010),(0.6*node_8441));
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 node_5958 = lerp(node_9770,(node_9770*_gloss_color),(max(0,dot(lightDirection,i.normalDir))*pow(max(0,dot(i.normalDir,halfDirection)),exp2(lerp(1,11,_gloss)))));
                float node_1010 = 0.0;
                o.Emission = lerp(node_5958,float3(node_1010,node_1010,node_1010),(0.6*node_8441));
                
                float3 diffColor = node_5958;
                float specularMonochrome;
                float3 specColor;
